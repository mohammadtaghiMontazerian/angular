...
@Injectable()
export class MediaItemServiceProxy {
    private http: HttpClient;
    private baseUrl: string;
    protected jsonParseReviver: ((key: string, value: any) => any) | undefined = undefined;
     ...
     * @return Success
     */
    get(id: number | undefined): Observable<MediaItemDto> {
        let url_ = this.baseUrl + "/api/services/app/MediaItem/Get?";
        if (id === null)
        ...
        return this.http.request("get", url_, options_).pipe(_observableMergeMap((response_ : any) => {
            ...
            if (response_ instanceof HttpResponseBase) {
                try {
                    ...
                    return _observableThrow(e) as any as Observable<MediaItemDto>;
                }
            } else
                return _observableThrow(response_) as any as Observable<MediaItemDto>;
        }));
    }

    protected processGet(response: HttpResponseBase): Observable<MediaItemDto> {
        const status = response.status;
        const responseBlob =
            response instanceof HttpResponse ? response.body :
            (response as any).error instanceof Blob ? (response as any).error : undefined;
        ...
        if (status === 200) {
            ...
            let result200: any = null;
            let resultData200 = _responseText === "" ? null : JSON.parse(_responseText, this.jsonParseReviver);
            result200 = MediaItemDto.fromJS(resultData200);
            return _observableOf(result200);
            }));
        } else if (status !== 200 && status !== 204) {
            return blobToText(responseBlob).pipe(_observableMergeMap((_responseText: string) => {
    ...
    protected processGetAll(response: HttpResponseBase): Observable<MediaItemDtoPagedResultDto> {
        const status = response.status;
        const responseBlob =
            response instanceof HttpResponse ? response.body :
            (response as any).error instanceof Blob ? (response as any).error : undefined;
     ...
     * @return Success
     */
    create(body: MediaItemDto | undefined): Observable<MediaItemDto> {
        let url_ = this.baseUrl + "/api/services/app/MediaItem/Create";
        url_ = url_.replace(/[?&]$/, "");
    ...
    protected processCreate(response: HttpResponseBase): Observable<MediaItemDto> {
        const status = response.status;
        const responseBlob =
            response instanceof HttpResponse ? response.body :
            (response as any).error instanceof Blob ? (response as any).error : undefined;
     ...
     * @return Success
     */
    update(body: MediaItemDto | undefined): Observable<MediaItemDto> {
        let url_ = this.baseUrl + "/api/services/app/MediaItem/Update";
        url_ = url_.replace(/[?&]$/, "");
    ...
    protected processUpdate(response: HttpResponseBase): Observable<MediaItemDto> {
        const status = response.status;
        const responseBlob =
            response instanceof HttpResponse ? response.body :
            (response as any).error instanceof Blob ? (response as any).error : undefined;
    ...
    delete(id: number | undefined): Observable<void> {
        let url_ = this.baseUrl + "/api/services/app/MediaItem/Delete?";
        if (id === null)
            throw new Error("The parameter 'id' cannot be null.");
        else if (id !== undefined)
            url_ += "Id=" + encodeURIComponent("" + id) + "&";
...
export class MediaItemDtoPagedResultDto implements IMediaItemDtoPagedResultDto {
    ...
    constructor(data?: IMediaItemDtoPagedResultDto) {
        if (data) {
            for (var property in data) {
                if (data.hasOwnProperty(property))
                    (<any>this)[property] = (<any>data)[property];
        ...
        if (_data) {
            if (Array.isArray(_data["items"])) {
                ...
                for (let item of _data["items"])
                    this.items.push(MediaItemDto.fromJS(item));
            }
            this.totalCount = _data["totalCount"];
        }
    }

    static fromJS(data: any): MediaItemDtoPagedResultDto {
        data = typeof data === 'object' ? data : {};
        let result = new MediaItemDtoPagedResultDto();
        result.init(data);
        return result;
    }
    ...
    clone(): MediaItemDtoPagedResultDto {
        const json = this.toJSON();
        let result = new MediaItemDtoPagedResultDto();
        result.init(json);
        return result;
...
export interface IMediaItemDtoPagedResultDto {
    items: MediaItemDto[] | undefined;
    totalCount: number;
}
